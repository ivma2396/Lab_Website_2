...
function loadStatsPage(){
	...
	for(row_counter = 0; row_counter < table.rows.length; row_counter++){
		for(col_counter = 0; col_counter < table.rows[row_counter].cells.length; col_counter++){
			...
			if (row_counter > 1 && col_counter == 2){
				homeScore = cell_value
			}
			if (row_counter > 1 && col_counter == 3){
				awayScore = cell_value
			}
			if (row_counter > 1 && col_counter == 4){
				if(parseInt(homeScore) > parseInt(awayScore)){
					cell_value = "CU Boulder";
	...
	document.getElementById("wins").innerHTML = CUWin
	document.getElementById("losses").innerHTML = CUloss
}
/*
...
function loadPlayersPage(){
	...
	for(i = 0; i < players.length; i++){
		str += "<a href='#' onclick='switchPlayers("+i+")'>" + players[i].name + "</a>"
		str += "</br>"		
	}
